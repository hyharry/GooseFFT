...
from mat_ten import stiffness_2_compliance_3333, tensor_2_m99
def tensor_flat_inv(vec):
    C_3333 = vec.reshape((3,3,3,3))
    # result = stiffness_2_compliance_3333(C_3333)
    result = stiffness_2_compliance_3333(C_3333, 'general')
    return result

def inv_K(K4):
    K_tmp = K4.reshape(-1,Nx*Ny*Nz)
    result = np.apply_along_axis(tensor_flat_inv,0,K_tmp)
    result = result.reshape((3,3,3,3,Nx,Ny,Nz))
    return result

apply_precond = lambda dF: np.einsum('ijklxyz,klxyz ->ijxyz',precond,dF.reshape((3,3,Nx,Ny,Nz)))
...
for inc in range(1,ninc):
    ...
    with np.printoptions(precision=3, suppress=True, linewidth=200):
        print(tensor_2_m99(precond[:,:,:,:,1,2,3]))
        print(tensor_2_m99(K4[:,:,:,:,1,2,3]))
        tmp = (tensor_2_m99(K4[:,:,:,:,1,2,3]))
        tmp2 = (tensor_2_m99(precond[:,:,:,:,1,2,3]))
        print(tmp.dot(tmp2))
    G_K_dF_x = lambda dF: G_K_dF(apply_precond(dF))
    # ***************
    ...
    while True:
        ...
        print(f'newton {iiter} end: |dF|/|F| = {dF_norm_rel:8.2e}, |rhs| = |G(P)| = {rhs_norm:8.2e}, ksp iter = {ksp_i[0]}')
